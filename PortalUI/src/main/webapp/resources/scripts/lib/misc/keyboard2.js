...
// ];
      ...
      for (var y = 0, keyid = "", ex; ex = elem[y++];) {
        if ((ex.nodeName == "TEXTAREA" || ex.type == "text" || ex.type == "password") && ex.className.indexOf("keyboardInput") > -1) {
          ...
          var keybut = document.createElement('img');
              keybut.src = pathKeyImage + "/common/image/keyboard.png";
  ...
  this.VKI_position = function() {
    if (self.VKI_visible != "") {
      ...
      self.VKI_keyboard.style.top = inputElemPos[1] - ((self.VKI_target.keyboardPosition == "fixed") ? document.body.scrollTop : 150) + self.VKI_target.offsetHeight + (Math.floor(Math.random( ) * (100+1))+ 100) + "px";
      self.VKI_keyboard.style.left = Math.min(VKI_innerDimensions()[0] - self.VKI_keyboard.offsetWidth - 150, inputElemPos[0])+ Math.floor(Math.random( ) * (100+1)) + "px";
    }
  };
...
function VKI_findPos(obj) {
  ...
  //alert("Cur left= " + curleft + "- cur top =" + curtop);
...
function nokeys(){
	 //false to disable input from keyboard
     //return false;
     
     // Request Production 2008/11/20 - enable input password from keyboard.
     return true;
}
